...
import { ScrollArea } from "@/components/ui/scroll-area";
import { User } from "lucide-react";
import { fetchCustomerByKeyword } from "@/redux/slices/customerSlice";
...
export default function CustomerSearchDropdown({
  ...
  // Restore input when a customer is selected / navigated back
  useEffect(() => {
    if (!selectedId) return;
    isTyping.current = false;
    setQuery(selectedName);
  ...
  // Debounced API search — same endpoint as All Customers
  useEffect(() => {
    if (!isTyping.current) return;
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if (fetchCustomerByKeyword.fulfilled.match(resultAction)) {
          ...
          const list = payload?.data ?? payload?.customers?.data ?? [];
