...
const EditUser = () => {
  ...
  useEffect(() => {
    if (!Array.isArray(selectedAdminPermissions)) return;

    const ids = selectedAdminPermissions.flatMap((group: any) =>
      (group?.permissions || []).map((p: any) => p?.id),
    );

    setValue("permissions", ids.filter(Boolean));
  }, [selectedAdminPermissions, setValue]);
  ...
  //         region: user.region || "",
  //         userRole: user.userRole ?? UserRolesEnum.ADMIN,
  //     }));
  // }, [userId, adminUsers]);
  useEffect(() => {
    ...
    reset((prev) => ({
      ...
      userRole: selectedAdminUser.userRole ?? UserRolesEnum.ADMIN,
    }));
    ...
    <div className="w-full min-h-screen bg-gray-100 flex flex-col">
      ...
      <div className="px-6 pt-6 pb-3">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Update the details of the user below. Assign the permissions this user
          should have access to, then click 'Save'.
        </p>
      </div>
                          ...
                          return (
                            <label
                              className={`flex items-center gap-3 px-3 py-2 cursor-pointer transition ${
                                isChecked ? "bg-[#eef2ff]" : "hover:bg-gray-50"
                              }`}
                            >
                              <Checkbox
                                checked={isChecked}
