...
export const fetchAdminUserById = createAsyncThunk(
  ...
  async ({ id }: { id: number }, thunkAPI) => {
    try {
      const res = await axiosInstance.get(`dashboard/admin-users/${id}`);
      return res.data;
    } catch (err: any) {
      return thunkAPI.rejectWithValue(
        err.response?.data?.message || "Failed to fetch admin user",
      );
    }
  },
);
  ...
  async ({ userId }: { userId: number }, thunkAPI) => {
    try {
      const res = await axiosInstance.get(
        `dashboard/permissions/user/${userId}`,
      );
      return res.data;
    } catch (err: any) {
      return thunkAPI.rejectWithValue(
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchAdminUserById.fulfilled, (state, action) => {
        ...
        state.selectedAdminPermissions = action.payload.permissions || [];
      })
